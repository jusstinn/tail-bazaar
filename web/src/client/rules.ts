  ...
  view.innerHTML = `
    <section class="band hero">
      <div class="wrap">
        ...
        <p class="lede reveal">A buyer pays for evidence it is not allowed to inspect first. That only works if somebody independent re-does the work and if the rules for releasing the payment are fixed before anyone pays. Both are written down here.</p>
        <p class="prose reveal">To watch it happen rather than read about it, the <a class="link-go" href="#/">marketplace</a> has a <strong>Buy</strong> button on every unsold finding and a <strong>List a new finding</strong> button for each robot: each runs the steps below live, one transaction at a time.</p>
      </div>
    </section>
    ...
    ${band({
      ...
      <div class="verdicts">${VERDICTS.map(([name, tone, money, what]) => `<div class="verdict reveal"><div class="verdict-name ${esc(tone)}">${esc(name)}</div><div><p class="verdict-money">${esc(money)}</p><p class="prose">${esc(what)}</p></div></div>`).join("")}</div>
      ${stateDiagram()}`,
    })}
    ...
    ${band({
      ...
      <div class="two-col">
        <div>
          <p class="prose reveal">Two deadlines are fixed at the moment the buyer funds the escrow and can never be changed afterwards. If the seller has not delivered by the delivery deadline, anyone may call <span class="mono">claimTimeout</span> and the buyer is refunded in full. If the verifier has not settled by the settlement deadline, the same is true. The seller therefore depends on a responsive verifier, and a verifier that goes quiet costs the seller, never the buyer.</p>
        </div>
        <div>
          <h3 class="sub reveal">Why a buyer cannot keep valid data <em>and</em> reclaim the payment</h3>
          <p class="prose reveal">A buyer who is unhappy calls <span class="mono">requestRecheck</span>. That function is restricted to the buyer of that listing and it does exactly one thing: it emits an event. It moves no funds and changes no state. The only function that moves money is the verifier's <span class="mono">settle</span>, and the contract allows exactly one terminal transition per listing. So a complaint is a public signal that the verifier must answer — not a refund the buyer can take for itself.</p>
        </div>
      </div>`,
    })}
