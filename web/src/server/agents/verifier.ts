...
//   5. if the environment fingerprint matches the seller's: the trajectory hash must be identical
...
//      same scenario, trajectory hash and claim as the verified run
//   7. publish a public summary (no parameters, no trajectory), register the listing on chain
...
export type VerificationResult = {
  ...
  verifier_run?: { outcome: string; trajectory_hash: string; metrics: Record<string, unknown>; file: string; environment: Record<string, unknown> };
...
export async function verifySubmission(sub: Submission, packageBytes: Uint8Array, log: (m: string) => void): Promise<VerificationResult> {
  ...
  const fail = (status: VerificationResult["status"], reason: string): VerificationResult => ({ status, reason, method: null, fingerprint_match: null, checks });
  ...
  const verifierRun = { outcome: v.doc.outcome, trajectory_hash: v.doc.trajectory_hash, metrics: v.doc.metrics, file: v.file, environment: v.doc.environment };
  ...
  if (fpMatch) {
    ...
    if (!check(checks, "trajectory-hash-identical", v.doc.trajectory_hash === sub.trajectory_hash, `${sub.trajectory_hash.slice(0, 18)} vs ${v.doc.trajectory_hash.slice(0, 18)}`))
      return { status: "INCONCLUSIVE", reason: "same environment fingerprint but different trajectory hash (numerical divergence or tampered run)", method, fingerprint_match: true, verifier_run: verifierRun, checks };
  } else {
    ...
    if (!check(checks, "metrics-within-tolerance", dImp <= TOLERANCE.impact_speed_mps && dT <= TOLERANCE.first_contact_t_s, `impact speed diff ${dImp.toFixed(4)} m/s (tol ${TOLERANCE.impact_speed_mps}), contact time diff ${dT.toFixed(4)} s (tol ${TOLERANCE.first_contact_t_s})`))
      return { status: "INCONCLUSIVE", reason: "different environment and metrics outside tolerance", method, fingerprint_match: false, verifier_run: verifierRun, checks };
  }
  ...
  return { status: "VERIFIED", reason: fpMatch ? "identical trajectory hash in a matching pinned environment" : "metrics within tolerance across differing environments", method, fingerprint_match: fpMatch, verifier_run: verifierRun, checks };
}
...
export type DeliveryCheck = {
  valid: boolean; reason: string; on_chain_commitment: string; delivered_hash: string; asserted_delivery_hash: string | null;
...
export async function verifierCheckDeliveryAndSettle(order: OrderRow, log: (m: string) => void): Promise<{ check: DeliveryCheck; tx: string }> {
  ...
  if (hashOk && isCanonical(delivered)) {
    ...
    const c5 = check(checks, "package-replay-matches-verified-run", pkg.replay?.trajectory_hash === verification.verifier_run?.trajectory_hash, `${String(pkg.replay?.trajectory_hash).slice(0, 14)}... vs verifier ${String(verification.verifier_run?.trajectory_hash).slice(0, 14)}...`);
    contentOk = c1 && c2 && c3 && c4 && c5;
  } else {
  ...
  const checkDoc: DeliveryCheck = {
    valid,
    reason: valid ? "delivered package hashes to the on-chain commitment and matches the advertised terms" : !hashOk ? "COMMITMENT MISMATCH: delivered bytes do not hash to the on-chain commitment" : "delivered package does not match the advertised terms",
